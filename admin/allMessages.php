...
$sql3 = mysql_query("SELECT * FROM messages ORDER BY date DESC LIMIT $page3,8");
$sql4 = mysql_query("SELECT * FROM messages ORDER BY date DESC");

$cou2 = mysql_num_rows($sql4);
$productCount2 = mysql_num_rows($sql3); // count the output amount
if ($productCount2 > 0) {
...
{ 
	...
	$usern1= $row["username"];
	$msg1 = $row["message"];
	$date_added1 = strftime("%b %d, %Y", strtotime($row["date"]));
	$product_list2 .= "
      <tr>
		<td align='left'>$usern1</td>
		<td align='left'>$msg1</td>
		<td align='left'>$date_added1</td>
		<td align='left' style='font-size:12pt;'><a href='message.php?deleteid=$id1' class='glyphicon glyphicon-remove'></a><br /></td>
      </tr>
";
}
...
{
	$product_list2 = "You have no messages yet";
}
		...
		<div class="container">
			...
			<div class="main">
				...
				<br>
					...
					<div style='font-size:15px;font:Kalinga;width:900px;margin:0 auto;' id='allMessages'>
					<h3 class='pull-left'>ALL MESSAGES</h3>
						...
						<?php for($b=1;$b<=$a2;$b++){ ?>
							<span style="border:2px #000 solid; color:#ff2800;text-align:center; margin:0 2.5px;padding:5px;"><a href="message.php?page2=<?php echo $b;?>" style="color:#ff2800;"><?php echo $b." ";?></a></span>
						<?php } ?>						
					</div>
