...
$sql = mysql_query("SELECT * FROM orders WHERE delivered='YES' ORDER BY date ASC LIMIT $page1,8");
$sql2 = mysql_query("SELECT * FROM orders WHERE delivered='YES' ORDER BY date ASC");
...
{
	$product_list1 = "You have not delivered any orders";
}
...
<?php 
$sql4 = mysql_query("SELECT * FROM orders WHERE delivered='NO'");
...
if(isset($POST['okid']))
{
	$sqlQuery3 = mysql_query("UPDATE orders SET delivered='YES' WHERE orderid = '$okid'");
}
?>
<!DOCTYPE html>
				...
				<header class="codrops-header" style='width:900px;'>
					...
					<a href="order.php" onclick='displayNewMessages()'>
						<div style='float:left;width:300px;height:100px;margin-right:100px;text-align:center;background-color:#ff2800;padding:5px;position:relative;'>
							...
							<div>NEW ORDERS</div>
							<span class='badge' style='position:absolute;top:10px;right:20px;'><?php echo $productCount2; ?></span>
						</div>
					</a>
					<a href="delivered.php" >					
						<div style='float:right;width:300px;height:100px;margin-right:100px;text-align:center;background-color:#ff2800;padding:5px;position:relative;'>
							...
							<span class='badge' style='position:absolute;top:10px;right:20px;color:#fff;font:Kalinga;'><?php echo $productCount1; ?></span>
						</div>
					</a>
					</div>
					...
					<h3 class='pull-left'>DELIVERED ORDERS</h3>
						<table class='table'>
							<thead>
							  <tr>
								<th>Username</th>
